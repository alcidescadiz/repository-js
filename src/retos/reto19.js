...
function learn(time = 0, courses = []) {
  let result = [];
  let alter = []
  courses.map((e, i) => {
    courses.map((x, j) => {
      if (x + e === time && i != j) result.push([i, j]);
      if (x + e <= time && i != j) alter.push([i, j, x+e]);
    });
  });
  alter = alter.filter(e=> e[2] < time).sort((e,x)=> x[2]-e[2]) 
  if (result.length === 0 && alter.length === 0) return null;
  if (result.length === 0 && alter.length > 0) return [alter[0][0], alter[0][1]];
  return result[0];
}
