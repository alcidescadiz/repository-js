...
export default function shouldBuyFidelity(times, Precio_entrada = 12) {
    let Entrada_normal= Precio_entrada * times
    let Tarjeta_fidelidad = 250
    for (let index = 1; index <= times; index++) {
        Tarjeta_fidelidad += Precio_entrada * (0.75**index)    
    }
    if(Entrada_normal > Tarjeta_fidelidad ){
        return true
    }else{
        return false
    }
...
console.log(shouldBuyFidelity(24)) // es factible a partir de 24 veces idas al cine